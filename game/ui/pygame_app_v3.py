...
class StartRewardScene:
    def __init__(
        ...
        self.fade_from_black_ms = 200 if (self.hp_loss > 0 and self._is_sigil_choice_scene()) else 0
    ...
    def _draw_sigil_choice_content(self, screen: pygame.Surface, sw: int, sh: int) -> None:
        ...
        header_h = 176
        header = pygame.Surface((content_w, header_h), pygame.SRCALPHA)
        ...
        _blit_text_outline(
            ...
            self.font_big,
            title,
            (content_x + content_w // 2, title_y + 24),
            fg=(255, 226, 170),
            outline=(8, 6, 6),
            center=True,
        )
        ...
        if not self.sigil_choices:
            self.continue_rect = pygame.Rect(content_x, title_y + header_h + 34, content_w, 92)
            ...
            _blit_text_outline(
                ...
                (self.continue_rect.centerx, self.continue_rect.y + 34),
                fg=(240, 235, 220),
                outline=(0, 0, 0),
                center=True,
            )
        ...
        icon_size = 96
        center_x = sw // 2
        top_y = title_y + header_h + 120
        horizontal_gap = min(220, max(160, content_w // 3))
        centers = [
            (center_x - horizontal_gap // 2, top_y),
            (center_x + horizontal_gap // 2, top_y),
            (center_x, top_y + 142),
        ]
        ...
        for sigil, center in zip(self.sigil_choices[:3], centers):
            ...
            if hovered:
                _draw_glow(screen, icon_rect.center, 78, (255, 255, 255), 42)
                _draw_glow(screen, icon_rect.center, 54, (255, 236, 164), 48)
                hovered_sigil = sigil

            panel_rect = icon_rect.inflate(18, 18)
            panel = pygame.Surface(panel_rect.size, pygame.SRCALPHA)
            pygame.draw.rect(panel, (0, 0, 0, 128), panel.get_rect(), border_radius=16)
            border_color = (255, 255, 255, 230) if hovered else (255, 230, 170, 90)
            pygame.draw.rect(panel, border_color, panel.get_rect(), width=(3 if hovered else 2), border_radius=16)
        ...
        hint_y = min(sh - 58, top_y + 250)
        hint_w = min(620, content_w)
        hint_x = (sw - hint_w) // 2
        hint_rect = pygame.Rect(hint_x, hint_y - 12, hint_w, 48)
        hint_surf = pygame.Surface(hint_rect.size, pygame.SRCALPHA)
        pygame.draw.rect(hint_surf, (0, 0, 0, 118), hint_surf.get_rect(), border_radius=12)
        screen.blit(hint_surf, hint_rect.topleft)
        _blit_text_outline(
            screen,
            self.font,
    ...
    def handle_click(self, pos: Tuple[int, int]) -> Optional[Dict[str, Any]]:
        if self._fade_from_black_alpha() > 0:
            return None
        if self.hp_loss > 0 and not self.damage_done:
            return None
