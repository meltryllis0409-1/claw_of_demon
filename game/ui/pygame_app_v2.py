...
class StartRewardScene:
    ...
    def _load_sigil_choice_icon(self, sigil: Sigil, size: int) -> Optional[pygame.Surface]:
        icon_size = (size, size)
        sigil_id = str(getattr(sigil, "sigil_id", ""))
        return (
            _load_image("ui", "sigils", f"{sigil_id}.png", size=icon_size)
            or _load_image("sigils", f"{sigil_id}.png", size=icon_size)
    ...
    def _draw_sigil_choice_content(self, screen: pygame.Surface, sw: int, sh: int) -> None:
        ...
        header = pygame.Surface((content_w, 150), pygame.SRCALPHA)
        ...
        if not self.sigil_choices:
            self.continue_rect = pygame.Rect(content_x, title_y + 184, content_w, 92)
        ...
        top_y = title_y + 250
        horizontal_gap = min(190, max(140, content_w // 3))
        centers = [
        ...
        _blit_text_outline(
            ...
            (content_x + 22, min(sh - 58, top_y + 246)),
            fg=(235, 230, 215),
            outline=(0, 0, 0),
        )
